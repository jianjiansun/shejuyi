(function(a) {
    a.fn.upload = function(D) {
        var r = a(this),
            ...
            g = '<svg xmlns="http://www.w3.org/2000/svg" class="delete" version="1" viewBox="0 0 1024 1024"></svg>',
            ...
            j, u, t, o, n, m, y, B = { height: r.attr("data-height") ? r.attr("data-height") : 0, width: r.attr("data-width") ? r.attr("data-width") : 1920, type: r.attr("data-type") ? r.attr("data-type") : "png,jpg,jpeg,gif", upname: r.attr("data-file") ? r.attr("data-file") : "file", inputname: r.attr("data-name") ? r.attr("data-name") : "upload", num: r.attr("data-num") ? r.attr("data-num") : 10, action: r.attr("action") ? r.attr("action") : "/index.php", size: r.attr("data-size") ? r.attr("data-size") : 20480, value: r.attr("data-value") ? r.attr("data-value") : "" };
        r.append(A.attr("data-num", B.num).attr("data-type", B.type)).append(d.attr("multiple", B.num > 1 ? "multiple" : false)).append(b.attr("name", B.inputname)).addClass(B.num > 1 ? "multiple" : "one").on("click", "li.add", function() { d.click() }).on("change", "input[type='file']", function(H) {
            var G = H.target.files;
            var i = [];
            for (var F in G) { if (typeof G[F] == "object") { i.push(G[F]) } }
            C(i)
        }).on("click", "li.error", function(i) { z(a(i.currentTarget)) }).on().on("click", "svg.look", function(i) { if (D && typeof D == "function") { D(r, a(i.currentTarget).parent("li.success").data("url")) } }).on("mousedown", "li.success", function(I) {
            var i = a(I.target);
            j = a(this);
            var J = j.offset();
            var H = r.offset();
        ...
        } else { r.data("num", 0).addClass("empty") }
        a(document).on("mousemove", function(G) {
            if (j) {
                ...
                o = G.pageX - H.left;
                n = G.pageY - H.top;
                if (n > (i + 20) || n < 0) { j.addClass("delete") } else { j.removeClass("delete") }
                o = parseInt(o / m);
                n = parseInt(n / y);
            ...
            if (j) {
                if (j.hasClass("delete") && j.data("filename")) {
                    z(j);
                    j = false;
                    r.children("li").not(".add").css({ "position": "relative", "top": 0, "left": 0, "margin-left": 0, "zIndex": 0 });
                ...
                if (o !== false) {
                    if (o >= r.children("li").size()) { i.insertBefore(A) } else { i.insertBefore(r.children("li").eq(o)) }
                    j.remove();
                    q();
                    r.children("li").not(".add").css({ "position": "relative", "top": 0, "left": 0, "margin-left": 0, "zIndex": 0 });
                    r.children("li.add").css("margin-left", 0)
            ...
            j = false
        }).on("dragenter", function(i) { i.preventDefault() }).on("dragover", function(i) { i.preventDefault(); if (a(i.target).hasClass("upload")) { r.addClass("dragenter") } else { r.removeClass("dragenter") } }).on("drop", function(H) {
            H.preventDefault();
        ...
        function l(G, L) {
            d.val("");
            if (B.num == 1) { r.children(".item").not(".add").remove() } else {
                if (B.num <= r.data("num")) { return false }
            ...
            var F = G.size,
                ...
                i = a(w);
            var I = B.type.split(",");
            if (I.indexOf(J) < 0) { var K = "不能上传." + J + "的文件!" }
            if (["png", "jpg", "jpeg", "gif", "bmp"].indexOf(J) < 0 && F > B.size * 1024) { var K = "不能上传大于." + B.size + "KB 的文件!" }
            if (K) {
            ...
            H.onload = function() {
                ...
                if (r.children('li[data-filename="' + P + '"]').size() > 0) { return false }
        ...
        function q() {
            var i = [];
            r.children("li.success").each(function() { i.push(a(this).data("url")) });
            if (r.children("li").not(".add").size() >= B.num && B.num > 1) { A.hide() } else { A.show() }
            if (r.children("li").not(".add").size() == 0) { r.addClass("empty") }
            b.val(i.join(","))
        }
