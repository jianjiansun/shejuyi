<?php
  ...
  header('content-type:text/html;charset=utf-8');
    ...
    //搜索社会组织
    public function searchOriganization()
    {
      //接收参数
      $search_body = I('get.search_body')?I('get.search_body'):""; //搜索框里输入的内容
      ...
      $type = I('get.type')?I('get.type'):"";  //二次筛选的条件
            ...
            $xs = new \XS('origanization'); // 建立 XS 对象，项目名称为：demo
            $search = $xs->search; // 获取 搜索对象
           // $docs = $search->search('郭淑青');
           // var_dump($docs);die;
      if(empty($type)){
              ...
              $docs = $search->setQuery($search_body)->addRange('sjy_origanization_type_id', $type, $type)->setLimit(10,$offset)->search();
              $count = $search->setQuery($search_body)->addRange('sjy_origanization_type_id', $type, $type)->count();
           }
           $ret['count'] = $count;
     ...
     //社会组织总数据数量
    public function getOriganizationCount()
    {
      //接收参数
      $search_body = I('get.search_body')?I('get.search_body'):""; //搜索框里输入的内容
