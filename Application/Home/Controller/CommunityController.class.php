...
class CommunityController extends BaseController {
        ...
        //换取项目服务对象
            ...
            foreach ($base64_data as $key => $value) {
                ...
                if (preg_match('/^(data:\s*image\/(\w+);base64,)/', $base64_image_content, $result)){
                $type = $result[2];
                $new_file = "./Uploads/community/projectimg/".date('Ymd',time())."/";
                        ...
                        if(!file_exists($new_file))
                        {
                         ...
                         if (file_put_contents($new_file, base64_decode(str_replace($result[1], '', $base64_image_content)))){
                                $url = "/Uploads/community/projectimg/".date('Ymd',time())."/".$filename.".{$type}";
                                $imgs[$key]['sjy_community_project_image'] = $url;
                                $imgs[$key]['sjy_community_project_id'] =$res;
            ...
            //插入项目图片
            $rut = M('community_project_image')->addAll($imgs);
            if($res)
            {
                $ret["state"] = 1;
                $ret['errorInfo'] = '';
